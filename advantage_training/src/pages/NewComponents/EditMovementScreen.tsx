  ...
  const handleUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (memberSnapshot.exists()) {
        ...
        const movementsScreen = memberData?.movementsScreen;
...
useEffect(() => {
    ...
    const memberRef: DocumentReference = doc(membersRef, memberId);
      ...
      if (memberSnapshot.exists()) {
        ...
        const movementScreenData :MovementScreen[]= Object.values(memberData?.movementsScreen || {});
        ...
        <Box display="flex" justifyContent="center">
          <form onSubmit={handleUpdate}>
            {movementScreen.map((screen) => (
              <Box key={screen.id} display="flex" flexDirection="column" alignItems="center">
            <TextField
  variant="outlined"
  ...
  <Select
    ...
    onChange={(event) =>
      setMovementScreen((prev) =>
        prev.map((item) =>
          item.id === screen.id
            ? { ...item, movementLevel: event.target.value as "Green" | "Yellow" | "Red" }
            : item
        )
      )
    }
