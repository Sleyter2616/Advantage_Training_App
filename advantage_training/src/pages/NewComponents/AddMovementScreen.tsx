...
const AddMovementScreenPage: React.FC = (
  ...
  const [values, setValues] = useState(defaultMovements.map((movement) => ({
    id:movement.id,
...
>
  ...
  <div key={value.id}>
    ...
    <FormControl className={classes.textField}>
      <InputLabel htmlFor={`movements.${index}.movementLevel`}>
      ...
      <Select
        ...
        onChange={(e) => {
          const value = e.target.value as "Yellow" | "Green" | "Red";
          setValues((prevMovements) => {
            const updatedMovements = prevMovements.map((movement, i) => {
              if (i === index) {
                return { ...movement, movementLevel: value };
              }
              return movement;
            });
            return updatedMovements;
        ...
        inputProps={{ name: `movements.${index}.movementLevel` }}
      >
