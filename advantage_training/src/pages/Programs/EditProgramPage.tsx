...
const EditProgramPage = () => {
  ...
  const handleMovementNameChange = (
    dayIndex: number,
    movementIndex: number,
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    const updatedDays = [...days];
    updatedDays[dayIndex].movements[movementIndex].movementName =
      event.target.value;
    setDays(updatedDays);
  };

  const handleMovementWeightChange = (
    dayIndex: number,
    movementIndex: number,
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    const updatedDays = [...days];
    updatedDays[dayIndex].movements[movementIndex].weight = String(
      event.target.value
    );
    setDays(updatedDays);
  };

  const handleMovementSetsChange = (
    dayIndex: number,
    movementIndex: number,
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    const updatedDays = [...days];
    updatedDays[dayIndex].movements[movementIndex].sets = String(
      event.target.value
    );
    setDays(updatedDays);
  };

  const handleMovementRepsChange = (
    dayIndex: number,
    movementIndex: number,
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    const updatedDays = [...days];
    updatedDays[dayIndex].movements[movementIndex].reps = String(
      event.target.value
    );
    setDays(updatedDays);
  };
  ...
  const handleDeleteProgram = async () => {
    try {
      ...
      const clientData = clientSnapshot.data();
      const updatedPrograms = clientData.programs.filter(
        (program: Program) => program.id !== programId
      );
            ...
            <Box key={dayIndex} mt={3}>
              ...
              <Box display="flex" justifyContent="center" mt={3}>
                <TextField
                  ...
                  value={day.dayNotes}
                  onChange={(event: React.ChangeEvent<HTMLInputElement>) =>
                    handleDayNotesChange(dayIndex, event)
                  }
                />
              </Box>
              <Table>
